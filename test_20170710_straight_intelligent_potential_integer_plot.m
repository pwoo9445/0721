...
othercars.car{2}.pos(1) = 35000;
othercars.car{1}.vel(1) = 15000;
othercars.car{2}.vel(1) = 15000;
...
%                 [mycar_est, U_rt, U_ft] = update_control_mycar_est(mycar_est, sim, othercars_est,laneChangePathTranslated,ratioSpeed, car_nr);
%                 U_r = U_r + U_rt;
%                 U_f = U_f + U_ft;
%                 if U_rt > U_rmax
%                    U_rmax = U_rt; 
...
%     %---後方車と各グリッドの相対角度・位置計算 by Yanagihara----------
    [theta_grid2me,rho_grid2me] = cart2pol(X - othercars.car{1}.pos(1),Y - othercars.car{1}.pos(2));
%     %-----------------------------------------------------------------
%     
%     %---後方車の動特性ポテンシャル計算 by Yanagihara-----------
    [direction_me, speed_me] = cart2pol(othercars.car{1}.vel(1)*cos(othercars.car{1}.pos(3)*pi/180),othercars.car{1}.vel(1)*sin(othercars.car{1}.pos(3)*pi/180));
