...
while sim.flag && ishandle(fig)
    ...
    switch key_pressed 
        ...
        case 'l'
            ...
            i_itr = 0;
            dec_max = 0;
            laneChangePathTranslated = update_laneChangePath(mycar_est,laneChangePath);
            ...
            if dec_max < 1000
                FLAG_LANECHANGE = 1;
            elseif dec_max < 500000
                FLAG_LANECHANGE_READY = 1;
            else
                FLAG_REARCAR_YUZURU = 0;
            end
            
            fprintf(1, 'MAXIMUM DECELERATION = [%d] \n', dec_max);
    ...
    switch sim.mode 
        case 'RUN'
            ...
            if FLAG_LANECHANGE == 1
              if FLAG_REARCAR_YUZURU == 1
                  othercars  = update_othercars_mycar_intelligent(othercars, sim, track, mycar, car_nr);
              else
            ...
            elseif FLAG_LANECHANGE_READY ==1
              i_itr = i_itr + 1;
              ...
              if i_itr == 100
                  ...
                  while mycar_est.pos(2) > -1700
                      %TTCを計算
                      [mycar_est, U_rt, U_ft] = update_control_mycar_intelligent_est(mycar_est, sim, othercars_est,laneChangePathTranslated,ratioSpeed, car_nr, front_num);
                      othercars_est = update_othercars(othercars_est, sim);
                      
                      
                      if -5000 < mycar_est.pos(1) - othercars_est.car{1}.pos(1) && mycar_est.pos(1) - othercars_est.car{1}.pos(1) < 5000
                          FLAG_LANECHANGE = 0;
                          FLAG_LANECHANGE_READY = 0;
                          break;
                      end
                  end
                  fprintf(1, 'DISTANCE = [%d] \n', mycar_est.pos(1) - othercars_est.car{1}.pos(1));
              end
...
%     %-------------------------------------------
    ...
    %----
    if FLAG_LANECHANGE == 1 || FLAG_LANECHANGE_READY == 1
        plot_winker(mycar);
    end
